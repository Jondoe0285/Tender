...
- Treat the existing Next.js application as the server/API implementation. Standing order: whenever a Buyer or Supplier SaaS workflow, copy, validation, API contract, payment, unlock, quote, accept, or contact-release path changes, update the Expo client in `mobile/` in the same change. Do not ship marketplace behaviour as browser-only. Owner, Super User, operator, and billing consoles stay web unless the user explicitly asks for them on mobile. This is source parity, not an automatic store or OTA publish.
