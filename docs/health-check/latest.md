...
**Migration result.** PASSED (schema validation: PASSED, production schema drift: FAILED)
...
### HC-20260830-010 — Production schema drift did not pass
...
**Summary.** The repository check "Production schema drift" reported FAILED. This is a release-blocking quality gate.

**Evidence.**

```
diff --git a/prisma/schema.sqlserver.prisma b/prisma/schema.sqlserver.prisma
index c24f4dd..b01ddea 100644
--- a/prisma/schema.sqlserver.prisma
+++ b/prisma/schema.sqlserver.prisma
@@ -299,7 +299,7 @@ model Tender {
   quantity      String
   urgency       String
...
**Approved-scope recommendation.** Only the files required to make "Production schema drift" pass.
**Expected files to change.** Within the approved scope above.
**Regression test required.** Yes
**Acceptance criteria.** "Production schema drift" completes with status PASSED.
**Validation commands.** `bash -lc npm run db:sync-prod-schema >/dev/null && git diff --exit-code -- prisma/schema.sqlserver.prisma`
**Dependencies.** None.
**Change risk.** Medium
...
Approved scope: Only the files required to make "Production schema drift" pass.
...
APPROVED SCOPE:
Only the files required to make "Production schema drift" pass.
...
`.github/workflows/deploy-azure.yml` declares `on: push: branches: [ main ]` with `environment: production`.

**Affected files.** `.github/workflows/deploy-azure.yml`
**Line numbers.** Not applicable.

**Reproduction steps.** Run the validation commands listed below on a clean checkout.
**User impact.** An unreviewed release could reach production users.
**Business impact.** Outage or data-integrity risk without an approval gate.
**Likely root cause.** See evidence.
**Recommended fix.** Configure required reviewers on the "production" GitHub environment, or split deployment into a separately dispatched workflow. This audit deliberately does not change deployment behaviour.
**Approved-scope recommendation.** Repository settings change, or an explicitly approved change to deploy-azure.yml.
**Expected files to change.** Within the approved scope above.
**Regression test required.** No — verified by repository settings evidence.
**Acceptance criteria.** A merge to main cannot reach production without a recorded human approval.
**Validation commands.** `(repository settings review — no local command)`
**Dependencies.** None.
**Change risk.** High
**Rollback considerations.** Changing deployment triggers affects release operations; coordinate with the release owner.
...
Approved scope: Repository settings change, or an explicitly approved change to deploy-azure.yml.
Expected files: .github/workflows/deploy-azure.yml
Required regression test: No — verified by repository settings evidence.
Acceptance criteria: A merge to main cannot reach production without a recorded human approval.
Maximum permitted change risk: High
Additional restrictions: Do not modify unrelated files, weaken tests, or change deployment behaviour.
...
Job-level `if: ${{ secrets.AZURE_WEBAPP_NAME != '' && secrets.AZURE_CREDENTIALS != '' }}` in `.github/workflows/deploy-azure.yml`.
...
**Reproduction steps.** Run the validation commands listed below on a clean checkout.
**User impact.** None directly.
**Business impact.** A deployment could attempt to run without valid credentials, or be skipped unexpectedly.
**Likely root cause.** See evidence.
**Recommended fix.** Move the secret presence check into a step that sets an output, or use a repository variable in the job condition.
**Approved-scope recommendation.** .github/workflows/deploy-azure.yml only, with release-owner approval because it affects deployment.
**Expected files to change.** Within the approved scope above.
**Regression test required.** No — workflow syntax validation only.
**Acceptance criteria.** The deployment job skips predictably when Azure credentials are absent.
**Validation commands.** `node scripts/health-check/validate-workflows.mjs`
**Dependencies.** None.
**Change risk.** Medium
**Rollback considerations.** Revert the fix commit; no data migration is involved.
...
Approved scope: .github/workflows/deploy-azure.yml only, with release-owner approval because it affects deployment.
Expected files: .github/workflows/deploy-azure.yml
Required regression test: No — workflow syntax validation only.
Acceptance criteria: The deployment job skips predictably when Azure credentials are absent.
Maximum permitted change risk: Medium
Additional restrictions: Do not modify unrelated files, weaken tests, or change deployment behaviour.
...
No previous archived audit was found; every finding is recorded as New.
...
3089de6 Update project status documentation
ac19503 Complete platform hardening workflows
007cfa6 Build Trade Tender procurement platform
2ce8268 Initial commit: UK construction tendering marketplace SaaS prototype
e6f1342 Initial commit
```

## 8. Recommended priorities

1. **HC-20260830-010** (HIGH) — Production schema drift did not pass
2. **HC-20260830-012** (HIGH) — No automated test covers Stripe webhook signature and idempotency
3. **HC-20260830-013** (HIGH) — No automated test covers Payment creation and charged totals
...
- `bash -lc set -e; DB="$PWD/health-check-migrate.db"; rm -f "$DB"; DATABASE_URL="file:$DB" npx prisma migrate deploy >/dev/null; set +e; DATABASE_URL="file:$DB" npx prisma migrate diff --from-url "file:$DB" --to-schema-datamodel prisma/schema.prisma --exit-code; STATUS=$?; rm -f "$DB"; exit $STATUS` → PASSED
- `bash -lc npm run db:sync-prod-schema >/dev/null && git diff --exit-code -- prisma/schema.sqlserver.prisma` → FAILED
