...
export function retailerInvitationTemplate(input: { companyName: string; inviteLink: string }): EmailTemplate {
  return {
    ...
    html: layout({ eyebrow: 'Provider account', title: 'Join the Trade Tender supply network', intro: `Your business has been invited to review qualified construction tender opportunities.`, body: detailRows([['Business', input.companyName], ['Access', 'Matched tender summaries and quote opportunities']]), action: { label: 'Complete provider setup', href: input.inviteLink } }),
  };
}
...
export function quoteAcceptedTemplate(input: { quoteReference: string; tenderReference: string; feeGbp: number; paymentPath: string }): EmailTemplate {
  return {
    ...
    html: layout({ eyebrow: 'Quote accepted', title: 'Your quote has been accepted', intro: 'The Contractor has selected your quote. Contact details will be released after the Accepted Quote Release Fee is confirmed.', body: detailRows([['Tender ID', input.tenderReference], ['Quote ID', input.quoteReference], ['Release fee', `£${input.feeGbp} excl. VAT`], ['Next step', 'The Contractor will complete the release payment in their workspace']]), action: { label: 'Continue in workspace', href: appUrl(input.paymentPath) } }),
  };
}
