...
export async function PATCH(request: Request, props: { params: Promise<{ id: string }> }) {
  try {
    ...
    const supportRequest = await reviewSupportRequest(reviewer, id, parsed.data.action, parsed.data.note, parsed.data.resolutionEvidence, parsed.data.triageCategory, parsed.data.escalationLevel);
