...
export async function PATCH(request: Request) {
  ...
  if (input.action === 'fee') {
    ...
    if (!['CLIENT_RELEASE_FEE_MODE', 'SPONSORED_PLACEMENT_ACTIVE', 'MEMBERSHIP_TIERS_ACTIVE', 'ADSPACE_ACTIVE', 'INDEPENDENT_REVIEW_ACTIVE', 'HUMAN_REVIEW_ACTIVE'].includes(input.key) && typeof input.value !== 'number') return NextResponse.json({ error: 'A numeric fee value is required' }, { status: 400 });
