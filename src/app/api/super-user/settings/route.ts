...
export async function PATCH(request: Request) {
  ...
  if (input.action === 'fee') {
    ...
    if (['RETAILER_UNLOCK_FEE_MODE', 'CLIENT_RELEASE_FEE_MODE', 'INDEPENDENT_REVIEW_PARTNER_URL', 'INDEPENDENT_REVIEW_SHARED_SECRET'].includes(input.key) && typeof input.value !== 'string') return NextResponse.json({ error: 'A string value is required' }, { status: 400 });
