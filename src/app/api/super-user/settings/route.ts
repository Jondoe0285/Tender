...
export async function PATCH(request: Request) {
  ...
  if (input.action === 'fee') {
    ...
    if (typeof input.value === 'number' && ['INDEPENDENT_REVIEW_FEE_BRONZE_GBP', 'INDEPENDENT_REVIEW_FEE_SILVER_GBP', 'INDEPENDENT_REVIEW_FEE_GOLD_GBP'].includes(input.key)) {
      const orderError = await assertIndependentReviewFeeOrder(input.key, input.value);
