...
export function SupportRequestQueue({ initialRequests, isOwner }: { initialRequests: Request[]; isOwner: boolean }) {
  ...
  return <div className="mx-auto max-w-5xl space-y-4">
    ...
    {requests.map((request) => <Card key={request.id}>
      ...
      <textarea aria-label={`Review note for ${request.title}`} value={notes[request.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [request.id]: event.target.value }))} className="mt-3 w-full rounded-md border border-slate-300 p-2 text-sm" placeholder="Triage note or information request" />
