...
export function SuperUserSettingsPanel({ initialSettings, isOwner }: { initialSettings: AdminSettings; isOwner: boolean }) {
  ...
  async function saveFee(key: 'RETAILER_UNLOCK_FEE_GBP' | 'RETAILER_UNLOCK_FEE_MODE' | 'RETAILER_UNLOCK_PERCENTAGE_LOW' | 'RETAILER_UNLOCK_PERCENTAGE_HIGH' | 'RETAILER_UNLOCK_PERCENTAGE_TOP' | 'CLIENT_RELEASE_FEE_GBP' | 'CLIENT_RELEASE_FEE_MODE' | 'CLIENT_RELEASE_PERCENTAGE_LOW' | 'CLIENT_RELEASE_PERCENTAGE_HIGH' | 'CLIENT_RELEASE_PERCENTAGE_TOP' | 'QUOTE_ESTIMATE_OFFSET_PERCENTAGE' | 'VAT_PERCENTAGE' | 'SPONSORED_PLACEMENT_ACTIVE' | 'SPONSORED_PLACEMENT_FEE_GBP' | 'MEMBERSHIP_TIERS_ACTIVE' | 'RETAILER_LAUNCH_CREDITS_DEFAULT' | 'ADSPACE_ACTIVE' | 'INDEPENDENT_REVIEW_ACTIVE' | 'INDEPENDENT_REVIEW_FEE_GBP' | 'HUMAN_REVIEW_ACTIVE', value: number | string | boolean) {
    setSaving(true);
    setMessage(null);
    try {
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Card><FieldEditor label="Internal estimate offset (%)" value={fees.quoteEstimateOffsetPercentage} onChange={(value) => setFees({ ...fees, quoteEstimateOffsetPercentage: value })} onSave={() => saveFee('QUOTE_ESTIMATE_OFFSET_PERCENTAGE', fees.quoteEstimateOffsetPercentage)} saving={saving} step="0.01" disabled={locked} /><p className="mt-3 text-sm text-concrete-grey">Applied internally to the baseline quote estimate, which refreshes from live quotation data available in the platform, to keep pricing aligned to current market conditions without exposing the estimate to Providers or Contractors.</p></Card>
