...
export type AdminSettings = {
  fees: { retailerUnlockGbp: number; clientReleaseGbp: number; clientReleaseMode: string; clientReleasePercentageLow: number; clientReleasePercentageHigh: number; sponsoredPlacementActive: boolean; sponsoredPlacementFeeGbp: number; membershipTiersActive: boolean; adspaceActive: boolean };
...
export function SuperUserSettingsPanel({ initialSettings, isOwner }: { initialSettings: AdminSettings; isOwner: boolean }) {
  ...
  async function saveFee(key: 'RETAILER_UNLOCK_FEE_GBP' | 'CLIENT_RELEASE_FEE_GBP' | 'CLIENT_RELEASE_FEE_MODE' | 'CLIENT_RELEASE_PERCENTAGE_LOW' | 'CLIENT_RELEASE_PERCENTAGE_HIGH' | 'SPONSORED_PLACEMENT_ACTIVE' | 'SPONSORED_PLACEMENT_FEE_GBP' | 'MEMBERSHIP_TIERS_ACTIVE' | 'ADSPACE_ACTIVE', value: number | string | boolean) {
    setSaving(true);
    setMessage(null);
    try {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2"><FieldGroup label="Name"><Input value={form.name} disabled={locked} onChange={(event) => setForm({ ...form, name: event.target.value })} /></FieldGroup><FieldGroup label="Monthly price (GBP)"><Input type="number" min="0" value={form.monthlyPriceGbp} disabled={locked} onChange={(event) => setForm({ ...form, monthlyPriceGbp: event.target.value })} /></FieldGroup><FieldGroup label="Free tender opportunities per month"><Input type="number" min="0" value={form.freeTenderOpportunitiesPerMonth} disabled={locked} onChange={(event) => setForm({ ...form, freeTenderOpportunitiesPerMonth: event.target.value })} /></FieldGroup><FieldGroup label="Description" wide><Textarea rows={3} value={form.description} disabled={locked} onChange={(event) => setForm({ ...form, description: event.target.value })} /></FieldGroup></div>
