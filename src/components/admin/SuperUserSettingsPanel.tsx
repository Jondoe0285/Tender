...
export function SuperUserSettingsPanel({ initialSettings, isOwner }: { initialSettings: AdminSettings; isOwner: boolean }) {
  ...
  async function saveFee(key: 'RETAILER_UNLOCK_FEE_GBP' | 'CLIENT_RELEASE_FEE_GBP' | 'CLIENT_RELEASE_FEE_MODE' | 'CLIENT_RELEASE_PERCENTAGE_LOW' | 'CLIENT_RELEASE_PERCENTAGE_HIGH' | 'CLIENT_RELEASE_PERCENTAGE_TOP' | 'VAT_PERCENTAGE' | 'SPONSORED_PLACEMENT_ACTIVE' | 'SPONSORED_PLACEMENT_FEE_GBP' | 'MEMBERSHIP_TIERS_ACTIVE' | 'RETAILER_LAUNCH_CREDITS_DEFAULT' | 'ADSPACE_ACTIVE' | 'INDEPENDENT_REVIEW_ACTIVE' | 'INDEPENDENT_REVIEW_FEE_GBP', value: number | string | boolean) {
    setSaving(true);
    setMessage(null);
    try {
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <Card><FieldEditor label="Provider tender unlock fee (excl. VAT)" value={fees.retailerUnlockGbp} onChange={(value) => setFees({ ...fees, retailerUnlockGbp: value })} onSave={() => saveFee('RETAILER_UNLOCK_FEE_GBP', fees.retailerUnlockGbp)} saving={saving} disabled={locked} /></Card>
