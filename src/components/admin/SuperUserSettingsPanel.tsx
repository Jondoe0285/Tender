    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Card><FieldEditor label="Bronze verification" value={fees.independentReviewFeeBronzeGbp} onChange={(value) => setFees({ ...fees, independentReviewFeeBronzeGbp: value })} onSave={() => saveFee('INDEPENDENT_REVIEW_FEE_BRONZE_GBP', fees.independentReviewFeeBronzeGbp)} saving={saving} disabled={locked} /></Card>
          <Card><FieldEditor label="Silver verification" value={fees.independentReviewFeeSilverGbp} onChange={(value) => setFees({ ...fees, independentReviewFeeSilverGbp: value })} onSave={() => saveFee('INDEPENDENT_REVIEW_FEE_SILVER_GBP', fees.independentReviewFeeSilverGbp)} saving={saving} disabled={locked} /></Card>
          <Card><FieldEditor label="Gold verification" value={fees.independentReviewFeeGoldGbp} onChange={(value) => setFees({ ...fees, independentReviewFeeGoldGbp: value })} onSave={() => saveFee('INDEPENDENT_REVIEW_FEE_GOLD_GBP', fees.independentReviewFeeGoldGbp)} saving={saving} disabled={locked} /></Card>
      ...
      <section>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          {SERVICE_NAMES.map((service) => {
            ...
            return <Card key={service}><h3 className="font-heading text-base font-bold text-foundation-navy">{service}</h3><div className="mt-3 space-y-3">{documents.map((document) => { const key = `${service}:${document.type}`; const required = verificationRequirements[key] === true; return <label key={key} className="flex items-start gap-3 text-sm text-concrete-grey"><input type="checkbox" checked={required} disabled={locked || saving} onChange={(event) => { const next = { ...verificationRequirements, [key]: event.target.checked }; void saveVerificationRequirements(next); }} className="mt-1 h-4 w-4 accent-trade-blue" /><span><span className="font-semibold text-foundation-navy">{document.label}</span><span className="block text-xs">{required ? 'Required for this service' : 'Optional for this service'}</span></span></label>; })}</div></Card>;
          })}
        </div>
      </section>
