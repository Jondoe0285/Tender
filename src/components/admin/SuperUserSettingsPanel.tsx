    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <Card><div className="flex flex-wrap items-center justify-between gap-3"><div><Label>Independent review renewal</Label><p className="mt-1 text-sm text-concrete-grey">Allows approved Providers to renew from month 11 at the lower renewal price before their independent verification expires.</p></div><Button variant={fees.independentReviewRenewalActive ? 'danger' : 'secondary'} disabled={locked} onClick={() => { const active = !fees.independentReviewRenewalActive; setFees({ ...fees, independentReviewRenewalActive: active }); void saveFee('INDEPENDENT_REVIEW_RENEWAL_ACTIVE', active); }} loading={saving}>{fees.independentReviewRenewalActive ? 'Deactivate' : 'Activate'}</Button></div><div className="mt-4"><FieldEditor label="Independent review renewal price (excl. VAT)" value={fees.independentReviewRenewalFeeGbp} onChange={(value) => setFees({ ...fees, independentReviewRenewalFeeGbp: value })} onSave={() => saveFee('INDEPENDENT_REVIEW_RENEWAL_FEE_GBP', fees.independentReviewRenewalFeeGbp)} saving={saving} disabled={locked} /></div></Card>
